...
public class CompanyTools {
    ...
    public Map<String, Object> getMonthlyProfit(String month) {
        String url = "http://localhost:8091/api/profit/" + month;
        Map<?, ?> profit = rt.getForObject(url, Map.class);
        // Return a simple map so the model can read it easily
        return Map.of(
                "month", profit.get("month"),
                "amountUsd", profit.get("amount")
        );
    }
